...
function renderSearch(c) {
    c.innerHTML = 
        '<h2 style="color:var(--gold);margin-bottom:15px">🔍 Discover</h2>' +
        '<input class="inp" id="sinput" placeholder="Search people..." onkeyup="searchUsers(this.value)" style="margin-bottom:12px">' +
        ...
        .then(function(snap) {
            ...
            users.forEach(function(u) {
                ...
                h += '<div class="chat-item" onclick="viewUserProfile(\'' + u.id + '\')">' +
                    '<img src="' + (u.data.avatar || defaultAvatar(u.data.name)) + '" class="chat-avatar-img" style="width:48px;height:48px" onerror="this.src=\'' + defaultAvatar(u.data.name) + '\'">' +
                    '<div style="flex:1"><b>' + u.data.name + '</b><br><small style="color:var(--gold-light)">' + u.data.username + '</small></div>' +
                    (isFollowing ? 
                        '<button class="btn-out" style="width:auto;padding:8px 16px" onclick="event.stopPropagation();startChatUser(\'' + u.id + '\')">Chat</button>' : 
                        '<button class="btn" style="width:auto;padding:8px 16px" onclick="event.stopPropagation();followUser(\'' + u.id + '\',this)">Follow</button>'
                    ) +
                    '</div>';
            });
            c.innerHTML = h;
...
function startChatUser(uid) {
    db.collection('chats').where('participants', 'array-contains', currentUser.uid).get().then(function(snap) {
        var ec = null;
        snap.forEach(function(doc) { 
            if (doc.data().participants.indexOf(uid) !== -1) ec = doc.id; 
        });
...
function viewUserProfile(userId) {
    db.collection('users').doc(userId).get().then(function(doc) {
        ...
        h += '<div style="display:flex;justify-content:center;gap:20px;margin:10px 0">' +
            '<div style="text-align:center"><b style="color:var(--gold)">' + (u.followers || []).length + '</b><br><small>Followers</small></div>' +
            '<div style="text-align:center"><b style="color:var(--gold)">' + (u.following || []).length + '</b><br><small>Following</small></div>' +
            '</div>';
        ...
        if (!isBlocked) {
            ...
            if (isFollowing) {
                ...
                h += '<button class="btn-out" style="flex:1;color:#FF4757;border-color:#FF4757" onclick="unfollowThisUser(\'' + userId + '\')">Unfollow</button>';
            } else {
                h += '<button class="btn" style="flex:1" onclick="followThisUser(\'' + userId + '\')">Follow</button>';
            }
            h += '</div>';
            h += '<button class="btn-out" style="color:#FF4757;border-color:#FF4757;margin-top:8px" onclick="blockUser(\'' + userId + '\')">🚫 Block</button>';
        } else {
            h += '<button class="btn" onclick="unblockUser(\'' + userId + '\')">✅ Unblock</button>';
        }
