...
// ✅ FIXED: openUserProfile ab selected user ka UID use karta hai
async function openUserProfile(uid) {
    console.log('Opening profile for UID:', uid); // Debug
    ...
    try {
        // ✅ SELECTED USER ko Firestore se load karo
        ...
        // ✅ Selected user ka data (NOT currentUserData)
...
function renderProfile(container, userData, uid) {
    ...
    // ✅ Compare UIDs to check if own profile
    ...
    container.innerHTML = `
        <div class="glass-panel" style="padding:1.5rem;text-align:center;position:relative;">
            ${!isOwnProfile ? `<button style="position:absolute;top:10px;right:10px;background:none;border:none;color:#ff4757;font-size:1rem;z-index:5;" onclick="window.reportUser('${uid}')" title="Report">⚠️</button>` : ''}
            ...
            ` : `
                <div style="display:flex;gap:0.5rem;justify-content:center;margin-top:1.5rem;flex-wrap:wrap;">
                    <button class="btn-glow" id="followBtn">${(window.currentUserData?.following||[]).includes(uid)?'<i class="fas fa-user-minus"></i> Unfollow':'<i class="fas fa-user-plus"></i> Follow'}</button>
...
async function togglePrivacy() {
    ...
    showToast(isPrivate?'Private account':'Public account', 'info');
}
...
async function toggleStatus() {
    ...
    showToast(hideStatus?'Status hidden':'Status visible', 'info');
}
