...
function togglePrivate() {
    var newStatus = !currentUserData.isPrivate;
    db.collection('users').doc(currentUser.uid).update({ isPrivate: newStatus }).then(function() {
        currentUserData.isPrivate = newStatus;
        showToast(newStatus ? 'Account is now Private 🔒' : 'Account is now Public 🌍');
        openPrivacy();
    });
}
...
function changePassword() {
    ...
    if (!old || !newP) return showToast('Fill all fields', 'error');
    if (newP.length < 6) return showToast('Min 6 chars', 'error');
...
function showBlockedUsers() {
    ...
    if (blocked.length === 0) {
        ...
        h += '<div id="blockedList">Loading...</div><button class="btn-out" onclick="openPrivacy()">Back</button>';
        document.getElementById('genericContent').innerHTML = h;
        var items = '', done = 0;
        blocked.forEach(function(id) {
            db.collection('users').doc(id).get().then(function(doc) {
                ...
                var u = doc.data();
                if (u) items += '<div class="chat-item"><div class="av" style="width:40px;height:40px">' + (u.name||'?')[0] + '</div><div style="flex:1"><b>' + u.name + '</b></div><button class="btn" style="width:auto;padding:6px 14px" onclick="unblockThisUser(\'' + id + '\')">Unblock</button></div>';
