...
function showLogin() {
    ...
    c.innerHTML = 
        '<div class="auth-box">' +
            ...
            '<h1 class="auth-title">ChronoX</h1>' +
            '<p class="auth-subtitle">PREMIUM SOCIAL NETWORK</p>' +
...
function showSignup() {
    ...
    c.innerHTML = 
        '<div class="auth-box">' +
            '<div style="font-size:55px">🕷️</div>' +
            '<h1 class="auth-title">Join ChronoX</h1>' +
            ...
            '<input class="inp" id="semail" placeholder="Email">' +
            '<input class="inp" id="spass" placeholder="Password (6+ chars)">' +
    ...
    db.collection('users').where('username', '==', '@' + u).get().then(function(snap) {
        ...
        auth.createUserWithEmailAndPassword(e, p).then(function(cred) {
            return db.collection('users').doc(cred.user.uid).set({
                ...
                achievements: [], streak: 0, bestStreak: 0, xp: 0, coins: 500,
                level: { current: 1, title: 'Explorer', progress: 0 },
